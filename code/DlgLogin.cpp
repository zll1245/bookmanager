...
{
	if (m_pCon == nullptr || m_pCon->State != adStateOpen) {
		MessageBox("数据库连接失败", "错误", MB_ICONERROR);
		return;
	}
	...
	CString sql;
	//CStringA c_userA(c_user);
	//CStringA c_passwordA(c_password);
	//MessageBox(c_user);
	//MessageBox(c_password);
	...
	if (!m_pRs->adoEOF)
	{
		...
		// 确认返回的记录
		//m_pRs->MoveFirst();
		CString db_name = m_pRs->GetCollect("name").bstrVal;
		CString db_password = m_pRs->GetCollect("password").bstrVal;

		// 输出数据库中查到的用户名和密码
		CString debug_msg;
		//debug_msg.Format("数据库中查到的用户名: %s，密码: %s", db_name, db_password);
		//MessageBox(debug_msg, "调试", MB_OK);
...
{
	CDialog::OnInitDialog();  // 必须调用基类，否则 DDX_Control 无效
	return TRUE;
}
